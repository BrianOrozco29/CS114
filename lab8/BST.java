...
implements Dictionary<Key, E> {
    ...
    public void printRang(BSTNode<Key, E> rt, Key lowK, Key highK)
    {
        if(rt == null)
            return;
            
        if(rt.key().compareTo(lowK) < 0)
            printRang(rt.right(), lowK, highK);
        else if(rt.key().compareTo(highK) > 0)
            printRang(rt.left(), lowK, highK);
        else
        {
            printRang(rt.left(), lowK, highK);
